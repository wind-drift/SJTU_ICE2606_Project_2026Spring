...
Default paths are now under the existing `data/` directory:

- Speech recordings and the task document: `data/raw/`
- Mixed-noise WAV generated by `noise.m`: `data/noise/mixed_noise.wav`
- Generated segments, checkpoints, metrics, and plots: `outputs/deep_learning/`

The expected raw speech filenames are `speaker1.mp3` through `speaker7.mp3`. The command-line option is still named `--reference-dir` for backward compatibility, but its default value is `data/raw`.
...
- Input: `1 x H x T` normalized feature map. With the current default MFCC frontend, `H = 39` for 13 MFCC plus delta and delta-delta.
...
All generated experiment files are written to `outputs/deep_learning/`. The repository ignores `outputs/` by default; this branch force-adds only the curated formal run artifacts in `outputs/deep_learning/`:
...
The current submitted run uses the MFCC frontend with delta and delta-delta features, `seed=42`, `epochs=120`, and `patience=20`. The checkpoint and generated artifacts are under `outputs/deep_learning/`.

- Clean accuracy: `0.95` on 20 validation samples.
- Mean mixed-noise accuracy across SNR `-10,-5,0,5,10,15,20` dB: `0.60`.
- Workflow score: `0.5 * clean + 0.5 * mean_noise = 0.775`.
- Mixed-noise SNR accuracy: `-10: 0.20`, `-5: 0.45`, `0: 0.60`, `5: 0.55`, `10: 0.75`, `15: 0.70`, `20: 0.95`.

Detailed artifacts:

- Clean metrics: `outputs/deep_learning/clean_metrics.json`
